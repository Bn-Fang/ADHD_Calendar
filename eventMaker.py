...
options = ["Eating", "Vyvance (12 hr)", "Study", "Sleep (8 hr)", "Work (2 hr)", "Exercise (30 min)", "Break", "Other", "Auto"]
...
def eventMaker():
    if st.session_state.loggedIn == True:
        ...
        if st.session_state.description == "Auto" or st.session_state.description == "Other":
            st.session_state.description = st.text_input("Enter Event Description")
            startDate= datetime.now()
            endDate= datetime.now()
            startTime= datetime.now().time().replace(microsecond=0)
            HrDelta = st.slider("Select Duration hrs", 0, 24, 1)
            MinDelta = st.slider("Select Duration mins", 0, 60, 0)
            endTime = (datetime.now() + timedelta(hours=HrDelta, minutes=MinDelta, seconds=0)).time().replace(microsecond=0)
